...
NamedScriptSync void PopulateRewards()
{
    ...
    for (i = 0; i < (MAX_DIFFICULTIES + 1); i++)
    {
        RewardsCount[i] = 0;
    }
...
NamedScript void BuildItemData()
{
    ...
    if (CompatMode == COMPAT_DRLA)
    {
        ...
        ITEMDATA_DEF("RLIncinerationDeviceLauncher",            "Incineration Device Launcher \Cv[Assembled]\C-",       180000, -1, -1, "NAPAX0", 22, 20);
...
ItemInfoPtr GetRewardItem(int Difficulty)
{
    ...
    // For some reason only Legendary (rarity 10) items fail here so here's a special hack for them until I can find the source. Maybe a GDCC bug?
    if (Difficulty == 10)
