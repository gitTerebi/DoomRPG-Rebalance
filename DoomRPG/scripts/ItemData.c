...
NamedScript void BuildItemData()
{
    ...
    ITEMDATA_DEF("DRPGClipBox", "Box of Bullets", 2000, 0, -1, "AMMOA0", 12, 16);
    ITEMDATA_DEF("DRPGShell", "Shells", 800, 0, -1, "SHELA0", 5, 7);
    ITEMDATA_DEF("DRPGShellBox", "Box of shells", 3000, 0, -1, "SBOXA0", 16, 12);
    ITEMDATA_DEF("DRPGRocketAmmo", "Rocket", 2500, 1, -1, "ROCKA0", 6, 27);
    ITEMDATA_DEF("DRPGRocketBox", "Box of Rockets", 10000, 1, -1, "BROKA0", 27, 20);
    ITEMDATA_DEF("DRPGCell", "Cells", 2000, 1, -1, "CELLA0", 8, 12);
    ITEMDATA_DEF("DRPGCellPack", "Cell Pack", 75000, 1, -1, "CELPA0", 15, 20);
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        for (int j = 0; j < ShieldPartsMax[i]; j++)
        {
            ...
            if (i == 0)
                ShieldPartCost += (j * 10 / 21) * j * 950; // 21 max index
            if (i == 1)
                ShieldPartCost += (j * 10 / 12) * j * 5000; // 12 max index
            if (i == 2)
                ShieldPartCost += (j * 10 / 14) * j * 4285; // 14 max index
            ...
            // Rarity
            int rarity = 1;
            if (ShieldPartCost <= 999999)
                rarity = 10;
            if (ShieldPartCost <= 200000)
                rarity = 9;
            if (ShieldPartCost <= 120000)
                rarity = 8;
            if (ShieldPartCost <= 75000)
                rarity = 7;
            if (ShieldPartCost <= 50000)
                rarity = 6;
            if (ShieldPartCost <= 35000)
                rarity = 5;
            if (ShieldPartCost <= 20000)
                rarity = 4;
            if (ShieldPartCost <= 15000)
                rarity = 3;
            if (ShieldPartCost <= 5000)
                rarity = 2;
            if (ShieldPartCost <= 2500)
                rarity = 1;
    ...
    for (int i = 0; i < MAX_ACCESSORIES; i++)
    {
        ...
        // Rarity
        int rarity = 1;
        if (Accessory->Price <= 999999)
            rarity = 10;
        if (Accessory->Price <= 200000)
            rarity = 9;
        if (Accessory->Price <= 120000)
            rarity = 8;
        if (Accessory->Price <= 90000)
            rarity = 7;
        if (Accessory->Price <= 50000)
            rarity = 6;
        if (Accessory->Price <= 40000)
            rarity = 5;
        if (Accessory->Price <= 30000)
            rarity = 4;
        if (Accessory->Price <= 20000)
            rarity = 3;
        if (Accessory->Price <= 15000)
            rarity = 2;
        if (Accessory->Price <= 8000)
            rarity = 1;
    ...
    if (CompatMode == COMPAT_DRLA)
    {
        ...
        // Common Armors
        ITEMDATA_DEF("RLGreenArmorPickup", "Security Armor \Cc[Common]\C- 15P 0M 15B 0F 0P", 250, 1, 0, "ARM1A0", 15, 17);
        ITEMDATA_DEF("RLBlueArmorPickup", "Combat Armor \Cc[Common]\C- 30P 0M 0B 0F 30P", 500, 1, 0, "ARM2A0", 15, 17);
...
int ShieldInversePriceSort(void const *Left, void const *Right)
{
    ...
    if (LeftPart->Price > RightPart->Price)
        return 1;
    if (LeftPart->Price < RightPart->Price)
        return -1;
...
ItemInfoPtr OptionalArgs(1) GetRewardItem(int Difficulty, bool SkipShieldPart)
{
    ...
    if (Difficulty < 10)
    {
        ...
        if (GetCVar("drpg_loot_rcm"))
        {
            ...
            if (DiffPick < (70.0 - 35.0 * MapLevelModifier))
                Difficulty--; // Unlucky, item will be a rank lower
            if (DiffPick > 95.0)
                Difficulty++; // Lucky, item will be a rank higher
        }
...
NamedScript DECORATE void SpawnLuckItem()
{
    ...
    if (Luck >= LUCK_AMMODROP && RandomFixed(0.0, 100.0) <= LUCK_MAXAMMOCHANCE)
        ActorToSpawn = "DRPGAmmoDropper";
    if (Luck >= LUCK_HEALTHDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXHEALTHCHANCE)
        ActorToSpawn = "DRPGHealthDropper";
    if (Luck >= LUCK_EPDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXEPCHANCE)
        ActorToSpawn = "DRPGEPDropper";
    // if (Luck >= LUCK_TURRETDROP  && RandomFixed(0.0, 100.0) <= LUCK_MAXTURRETCHANCE)  ActorToSpawn = "DRPGTurretDropper";
    if (Luck >= LUCK_MODULEDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXMODULECHANCE)
        ActorToSpawn = "DRPGModuleDropper";
    if (Luck >= LUCK_ARMORDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXARMORCHANCE)
        ActorToSpawn = "DRPGArmorDropper";
    if (Luck >= LUCK_WEAPONDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXWEAPONCHANCE / 3)
        ActorToSpawn = "DRPGWeaponDropper";
    if (Luck >= LUCK_AUGDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXAUGCHANCE)
        ActorToSpawn = "DRPGAugDropper";
    if (Luck >= LUCK_SHIELDDROP && RandomFixed(0.0, 100.0) <= LUCK_MAXSHIELDCHANCE)
        ActorToSpawn = "DRPGShieldDropper";
    if (Luck >= LUCK_EPDROP && RandomFixed(0.0, 100.0) <= 1.00)
        ActorToSpawn = "DRPGChipDropper";
    if (Luck >= LUCK_TURRETDROP && RandomFixed(0.0, 100.0) <= 1.00)
    ...
    if (!CurrentLevel->UACBase && Random(0, 250) <= 75 + RoundInt(7.5 * (fixed)Modifier))
    {
        ...
        while (!ItemSpawned)
        {
            for (int j = 0; j < ItemMax[ItemCategory]; j++)
            {
                ...
                if (CompatMode == COMPAT_DRLA)
                {
                    if (!CommonChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 9, 6) == "Common")
                        continue;
                    if (!ExoticChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 9, 6) == "Exotic")
                        continue;
                    if (!SuperiorChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 11, 8) == "Superior")
                        continue;
                    if (!UniqueChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 9, 6) == "Unique")
                        continue;
                    if (!DemonicChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 10, 7) == "Demonic")
                        continue;
                    if (!LegendaryChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 12, 9) == "Legendary")
                        continue;
                }
        ...
        while (!ItemSpawned)
        {
            for (int j = 0; j < ItemMax[ItemCategory]; j++)
            {
                ...
                if (CompatMode == COMPAT_DRLA)
                {
                    if (!CommonChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 9, 6) == "Common")
                        continue;
                    if (!AssembledChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 12, 9) == "Assembled")
                        continue;
                    if (!ExoticChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 9, 6) == "Exotic")
                        continue;
                    if (!SuperiorChance && StrMid(ItemData[ItemCategory][j].Name, StrLen(ItemData[ItemCategory][j].Name) - 11, 8) == "Superior")
